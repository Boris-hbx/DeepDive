...
# 追问：还要结合最新技术的发展思考未来的发展趋势，例如AI agent的身份与零信任机制会向什么方向发展
...
**基于 LLM 的策略推理引擎。** 传统 Policy Engine（如 OPA/Rego）依赖人工编写的声明式策略。
